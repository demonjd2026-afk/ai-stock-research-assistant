...
# ------------------------------------------------------------------
# 0. Clients — SDK picks up DATABRICKS_HOST + DATABRICKS_TOKEN
#    automatically when running as a Databricks App
# ------------------------------------------------------------------
w = WorkspaceClient()
...
# ------------------------------------------------------------------
# 2. Tool implementations — read from Gold, write to main.agent
# ------------------------------------------------------------------
    ...
    try:
        ...
        rows = run_sql(f"""
            ...
            WHERE LOWER(title) LIKE '%{query.lower()}%'
               OR LOWER(COALESCE(description,'')) LIKE '%{query.lower()}%'
            LIMIT {num_results}
        """)
        return rows if rows else [{"note": f"Fallback search. Error: {str(e)[:80]}"}]
...
def save_analysis_report(ticker: str, report_text: str) -> dict:
    ...
    run_sql(f"""
        ...
        VALUES ('{uuid.uuid4()}', '{USER_EMAIL}', '{ticker.upper()}',
                '{clean}', '{MODEL}', '{datetime.now().isoformat()}')
    """)
...
# ------------------------------------------------------------------
TOOLS = [
    {"type":"function","function":{"name":"get_price_data","description":"Get current price data and metrics for a stock ticker","parameters":{"type":"object","properties":{"ticker":{"type":"string","description":"Stock ticker e.g. AAPL"}},"required":["ticker"]}}},
    ...
    {"type":"function","function":{"name":"compare_tickers","description":"Compare multiple tickers side by side on price and sentiment","parameters":{"type":"object","properties":{"tickers":{"type":"array","items":{"type":"string"}}},"required":["tickers"]}}},
    {"type":"function","function":{"name":"get_top_movers","description":"Get today's top gaining and losing stocks","parameters":{"type":"object","properties":{"limit":{"type":"integer","default":5}}}}},
    {"type":"function","function":{"name":"search_news","description":"Semantic search over recent stock news articles","parameters":{"type":"object","properties":{"query":{"type":"string"},"num_results":{"type":"integer","default":3}},"required":["query"]}}},
    {"type":"function","function":{"name":"add_to_watchlist","description":"Add a stock ticker to user watchlist","parameters":{"type":"object","properties":{"ticker":{"type":"string"},"watchlist_name":{"type":"string","default":"My Watchlist"}},"required":["ticker"]}}},
    {"type":"function","function":{"name":"save_research_note","description":"Save a research note for a ticker","parameters":{"type":"object","properties":{"ticker":{"type":"string"},"note":{"type":"string"}},"required":["ticker","note"]}}},
    {"type":"function","function":{"name":"save_analysis_report","description":"Save a full analysis report for a ticker","parameters":{"type":"object","properties":{"ticker":{"type":"string"},"report_text":{"type":"string"}},"required":["ticker","report_text"]}}}
]
...
Be concise, data-driven, and proactively suggest saving notes or adding to watchlist."""
...
# ------------------------------------------------------------------
# 5. Market summary helpers for sidebar
# ------------------------------------------------------------------
def load_market_summary() -> str:
    try:
        rows = run_sql("""
            SELECT ticker, name, close, daily_return_pct, is_up_day, sentiment_signal
            FROM main.gold.ticker_daily_summary t
            LEFT JOIN main.gold.sentiment_summary s USING (ticker)
            ORDER BY daily_return_pct DESC
        """)
        if not rows:
        ...
        for r in rows:
            ...
            ret    = r.get("daily_return_pct", 0)
            signal = r.get("sentiment_signal", "")
            sentiment_emoji = "📈" if signal == "BULLISH" else ("📉" if signal == "BEARISH" else "➡️")
            lines.append(
                f"{arrow} **{r['ticker']}** ${r['close']}  "
                f"({ret:+.2f}%)  {sentiment_emoji}"
            )
        return "\n".join(lines)
    except Exception as e:
...
def load_watchlist() -> str:
    try:
        rows = run_sql(f"""
            SELECT ticker, watchlist, added_at
        ...
        lines = ["**📋 Watchlist**\n"]
        for r in rows:
            lines.append(f"• **{r['ticker']}** — {r['watchlist']}")
        return "\n".join(lines)
    except Exception as e:
...
def chat(message: str, history: list) -> tuple[str, list, str]:
    ...
    trace_md = ""
    if tool_trace:
        trace_lines = ["**🔧 Tools called:**\n"]
        for t in tool_trace:
            args_str   = json.dumps(t["args"])
            result_str = json.dumps(t["result"], default=str)[:150]
            trace_lines.append(f"→ `{t['tool']}({args_str})`")
            trace_lines.append(f"← `{result_str}...`\n")
        trace_md = "\n".join(trace_lines)
    ...
    *Powered by Databricks Lakehouse + Meta Llama 3.3 70B*
    ...
    with gr.Row():
        ...
        with gr.Column(scale=3):
            chatbot = gr.Chatbot(
                label  = "Chat with your Stock AI",
                height = 480,
                show_copy_button = True
            )
            with gr.Row():
                msg = gr.Textbox(
                    placeholder = "Ask about stocks, prices, news, sentiment...",
                    show_label  = False,
                    scale       = 4
                )
            ...
            tool_trace_box = gr.Markdown(label="Tool trace", value="")

        # Right — sidebar
        with gr.Column(scale=1):
            market_md = gr.Markdown(load_market_summary())
            gr.Button("🔄 Refresh Market", size="sm").click(
                fn      = load_market_summary,
                outputs = market_md
            )

            gr.Markdown("---")

            watchlist_md = gr.Markdown(load_watchlist())
            gr.Button("🔄 Refresh Watchlist", size="sm").click(
                fn      = load_watchlist,
                outputs = watchlist_md
            )
    ...
    send_btn.click(
        fn      = chat,
        inputs  = [msg, chatbot],
        outputs = [msg, chatbot, tool_trace_box]
    )
    msg.submit(
        fn      = chat,
        inputs  = [msg, chatbot],
